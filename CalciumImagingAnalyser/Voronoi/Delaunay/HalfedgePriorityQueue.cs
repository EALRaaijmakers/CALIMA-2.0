...
namespace csDelaunay {
	...
	public class HalfedgePriorityQueue {
		...
		private int Bucket(Halfedge halfedge) {
			int theBucket;
			/*if (float.IsInfinity (halfedge.ystar) || float.IsNaN (halfedge.ystar)) {
				theBucket = 0;
			} else {
				theBucket = (int)((halfedge.ystar - ymin)/deltaY * hashSize);
			}*/
			try {
			 theBucket = (int)((halfedge.ystar - ymin)/deltaY * hashSize);
			} catch (Exception e) {
				//Console.WriteLine (e.Message);
				//Console.WriteLine ("deltaY*hashsize:" + deltaY * hashSize);
				theBucket = 0;
			}
